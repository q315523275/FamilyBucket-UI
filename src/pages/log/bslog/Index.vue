    ...
    return {
      ...
      columns: [
        ...
        },
        {
          ...
          width: 160,
          render: (row, column) => {
            return <span>{ this.$utils.Date.format(parseInt(row.REPTIME / 1000)) }</span>
          }
        },
        {
          prop: 'REPTIMESPAN',
        ...
        list: [
          {
            ...
            method: (index, row) => {
              this.form = {
                ...
                REQPARAM: JSON.stringify(JSON.parse(row.REQPARAM), null, 4),
                REPVALUE: JSON.stringify(JSON.parse(row.REPVALUE), null, 4)
              }
